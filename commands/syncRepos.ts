  ...
  const results = await processReposParallel(
    async (repo) => {
      ...
      try {
        ...
        const currentBranch = (
          await runGitAsync(repo, ['branch', '--show-current'], {
            silent: true,
          })
        ).stdout.trim();
        ...
        if (success) {
          ...
          const error =
            stashError || `Pull failed with status ${pullRes.status}`;
          updateRepoState(repo, {
    ...
    },
    {
      ...
      filterPromptTitle: options.fetchOnly
        ? 'Select repositories to fetch'
  ...
  if (results?.length) {
    ...
    if (failures.length) {
      ...
      console.log(
        '\n' +
          chalk.green.bold(
            `All repositories ${options.fetchOnly ? 'fetched' : 'synced'} successfully!`,
          ),
      );
    }
  }
