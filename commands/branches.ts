...
function parseBranchVV(output: string): { name: string; raw: string; isCurrent: boolean; hasUpstream: boolean; isGone: boolean }[] {
    const lines = output
        .split("\n")
        .map((l) => l.trimEnd())
        .filter((l) => l.length > 0);
    ...
    return lines.map((line) => {
        ...
        return { name, raw: line, isCurrent, hasUpstream: hasBracket && !isGone ? true : hasBracket && isGone ? true : hasBracket, isGone };
    });
}
...
export async function branches() {
    await processReposParallel(async (repo) => {
        const res = await runGitAsync(repo, ["branch", "-vv", "--no-color"], { silent: true });
