...
export async function scheduleCommand(args: string[]) {
    ...
    switch (sub) {
        ...
        default:
            ...
            console.log("  run    - Manually trigger the sync logic (used by the scheduler)");
            break;
    }
}
...
function setupWindows(command: string) {
    ...
    const res = spawnSync("schtasks", ["/create", "/tn", taskName, "/tr", command, "/sc", "daily", "/st", "10:00", "/f"], {encoding: "utf-8"});
    if (res.status === 0) {
        console.log("✅ Windows Task Scheduler entry created successfully (Daily at 10:00 AM).");
    } else {
...
<dict>
    ...
    <array>
        ${command.split(" ").map(arg => `<string>${arg.replace(/"/g, "")}</string>`).join("\n        ")}
    </array>
    ...
    try {
        ...
        if (res.status === 0) {
            console.log("✅ macOS LaunchAgent created successfully (Daily at 10:00 AM).");
        } else {
...
function setupLinux(command: string) {
    ...
    try {
        ...
        if (existing.status === 0) {
            newCron = existing.stdout.split("\n").filter(line => !line.includes("gml schedule run")).join("\n");
        }
        ...
        if (res.status === 0) {
            console.log("✅ Linux crontab entry created successfully (Daily at 10:00 AM).");
        } else {
...
async function removeSchedule() {
    const os = platform();
    if (os === "win32") {
        const res = spawnSync("schtasks", ["/delete", "/tn", "GML_Daily_Sync", "/f"], {encoding: "utf-8"});
        ...
        const existing = spawnSync("crontab", ["-l"], {encoding: "utf-8"});
        if (existing.status === 0) {
            const newCron = existing.stdout.split("\n").filter(line => !line.includes("gml schedule run")).join("\n");
