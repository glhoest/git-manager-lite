...
export async function switchToMain() {
    ...
    for (const repo of selected) {
        ...
        console.log(`[${repo}] Current branch: `, currentBranch || "(detached)", '\n');
            ...
            try {
                const resp = await prompts({
                    ...
                    choices: [
                        ...
                        { title: 'Stash changes, switch, then try to unstash (safe)', value: 'stash' },
                ...
                if (action === 'abort') { abortAll = true; break; }
                if (action === 'skip') { console.log(`[${repo}] Skipped.`); continue; }
        ...
        if (mode === 'stash') {
            const res = stashSave(repo, `gml: auto-stash before switching to ${mainBranch}`);
            if (!res.ok) {
                console.error(`[${repo}] Failed to stash changes. Aborting for safety.`);
                abortAll = true;
                break;
            }
            stashRef = res.ref;
        ...
        if (mode === 'stash' && stashRef) {
            ...
            if (applied.ok) {
                ...
                console.warn(`[${repo}] Failed to apply ${stashRef}. Your changes are safely kept in stash.`);
    ...
    if (unstashFailures.length) {
        ...
        console.log("You can manually apply the stash later using 'git stash list' and 'git stash apply <ref>'.");
    }
}
