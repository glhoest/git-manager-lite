# Quick wins
- [x] Switch to main command should filter out repos already on main
- [x] Sync command should end with a recap of failed repos
- [x] Sync command fetch only mode ( --fetch-only, and alias 'fetch' instead of sync)
- [x] include common typos as aliases for commands (e.g. 'sl' as type for 'ls')

# Improvements
- [x] List command should show status for current branch
- [x] Sync command should process parallel
- [x] list command should process parallel
- [x] branches cleanup command should have a flag to operate on the remote, this should add extra safety checks
- [x] Branch management mode: select a repo, explore local branches and remote branches, filter, select a branch and switch to it
